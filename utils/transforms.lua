...
function M.Affine(deg)
    return function(input)
        ...
        input = image.affinetransform(input:float(), affineMat)
        return input
    end
end
...
function M.Smooth(size)
    local kernel = image.gaussian(size)
    return function (input)
        kernel = kernel:typeAs(kernel)
        return image.convolve(input, kernel, 'same')
    end
end

function M.RandomSmooth(sizes)
    return function (input)
        local size = sizes[torch.random(1,#sizes)]
        local kernel = image.gaussian(size):typeAs(input)
        return image.convolve(input, kernel, 'same')
    end
end
